---
name: publish-deploy
description: End-to-end workflow for pushing a web project to GitHub and deploying it live on Vercel, GitHub Pages, or FTP through GitHub Actions. Use when user says "publish", "deploy", "make live", "push", "go live", "host this", "put it online", or wants FTP deploy. Handles static assets, env vars, build errors, Actions secrets, and SPA routing gotchas.
---
...
> ⚠️ **Gotcha:** `gh repo create` will fail with "Unable to add remote" if `origin` already exists.
...
# 3. Future updates (auto-deploy enabled):
...
Use when the project should deploy to classic FTP hosting after every push.
...
For GitHub Pages, prefer the one-command script:

```bash
../deployment_templates/scripts/publish-github-pages.sh my-site private auto
```

This detects Vite/Next/static, adds a Pages workflow, creates the repo, pushes to `main`, and tries to enable Pages through the GitHub API. User interaction should only be needed for first-time `gh auth login`, 2FA, or missing GitHub token scopes.
...
../deployment_templates/scripts/init-github-repo.sh my-site private
../deployment_templates/scripts/set-ftp-secrets.sh
git add -A
...
../deployment_templates/scripts/init-github-repo.sh my-next-site private
../deployment_templates/scripts/set-ftp-secrets.sh
git add -A
git commit -m "ci: add ftp deploy"
git push origin main
...
../deployment_templates/scripts/init-github-repo.sh my-static-site private
../deployment_templates/scripts/set-ftp-secrets.sh
git add -A
git commit -m "ci: add ftp deploy"
git push origin main
...
Never commit FTP passwords. Store them only in GitHub Secrets.
