...
pdf_clean_canvas = '/tmp/clean_canvas.pdf'
...
doc = fitz.open(pdf_clean_canvas)
...
# Wire types text next to icons
p1.insert_text((72.0, 843.0), "Drut (Solid Wire)", fontname="freg", fontsize=8.5, color=(0.137, 0.122, 0.125))
p1.insert_text((180.0, 843.0), "Linka (Strand Wire)", fontname="freg", fontsize=8.5, color=(0.137, 0.122, 0.125))
...
# Table Headers (white text on blue header bar)
...
for row in rows:
    p4.insert_text((65.0, row["light_y"]), row["cat_no"], fontname="fbold", fontsize=9.1, color=(0.137, 0.122, 0.125))
...
# Save output
tmp_out = '/tmp/translated_clean_build.pdf'
doc.save(tmp_out)

shutil.copy(tmp_out, pdf_out_orig)
shutil.copy(tmp_out, pdf_out_pl)

# Render PNG previews for verification
out_dir = '/Users/karolbohdanowicz/.gemini/antigravity-ide/brain/c845d1d1-33ac-44db-b334-e418870bc328/clean_built_preview'
...
print("Clean built PDF finished successfully!")
