document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function processUserInput(text) {
    ...
    if (aiSessionState.awaitingClarification === 'confirm_length') {
        if (/tak|t|yes|y/i.test(lowerText)) {
            aiSessionState.length = aiSessionState.pendingLength;
            aiSessionState.pendingLength = null;
            aiSessionState.awaitingClarification = null;
        } else if (/nie|n|no/i.test(lowerText)) {
            aiSessionState.pendingLength = null;
            aiSessionState.awaitingClarification = 'length';
            streamText(aiBubble, "Rozumiem. W takim razie ile dokładnie metrów potrzebujesz?");
            return;
        } else {
            // User typed something else entirely, let's treat it as a new input
            aiSessionState.awaitingClarification = null;
        }
    }
    ...
    if (aiSessionState.application) {
        if (!aiSessionState.length) {
            if (aiSessionState.pendingLength) {
                aiSessionState.awaitingClarification = 'confirm_length';
                streamText(aiBubble, `Czy chodziło Ci o **${aiSessionState.pendingLength} metrów**?`, () => {
                    ...
                    btnTak.className = 'add-to-cart-btn';
                    btnTak.style.flex = '1';
                    btnTak.style.padding = '10px';
                    btnTak.style.border = 'none';
                    btnTak.textContent = 'Tak';
                    btnTak.onclick = () => {
                        btnContainer.remove();
                        const inputField = document.getElementById('aiChatInput');
                        if (inputField) {
                            inputField.value = 'Tak';
                            document.querySelector('.ai-input-area button').click();
                        }
                    };

                    const btnNie = document.createElement('button');
                    btnNie.className = 'buy-it-now-btn';
                    btnNie.style.flex = '1';
                    btnNie.style.padding = '10px';
                    btnNie.textContent = 'Nie';
                    btnNie.onclick = () => {
                        btnContainer.remove();
                        const inputField = document.getElementById('aiChatInput');
                        if (inputField) {
                            inputField.value = 'Nie';
                            document.querySelector('.ai-input-area button').click();
                        }
                    };
        ...
        if (!aiSessionState.light) {
            ...
            streamText(aiBubble, `Super, mamy ustalone **${aiSessionState.length}m** do **${ConfiguratorCore.labels?.application?.[aiSessionState.application] || 'tego pomieszczenia'}**. Jaką barwę światła wolisz? (np. Ciepła, Neutralna, Zimna, lub Kolorowa)`);
            return;
        }
        ...
        if (!candidates.length) {
            streamText(aiBubble, "Przykro mi, ale przy tych specyficznych wymaganiach nie potrafię dobrać idealnej taśmy. Spróbujmy od nowa. Zmieńmy może barwę lub technologię. Co chcesz zmienić?");
            return;
        }
    ...
    // Generic Fallback
    streamText(aiBubble, "Nie do końca Cię zrozumiałem. Będę mógł pomóc najskuteczniej, jeśli opiszesz szczegółowo co chcesz oświetlić, np: 'Potrzebuję taśmy led do salonu na sufit, około 10m, barwa ciepła'.");
