        ...
        {activeTab === 'home' && (
          <section className="view-section active">
            ...
            <div className="section-container" style={{ padding: '10px 15px' }}>
              <h2 className="section-title" style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '20px', textAlign: 'center' }}>Gdzie sprawdzają się zasilacze?</h2>
              <div className="app-grid-m" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
                {[
                  { title: 'Domy i Rezydencje', desc: 'Oświetlenie elewacji, podbitek i schodów.', img: '/assets/app_domy_1783188239361.png' },
                  { title: 'Bloki mieszkalne', desc: 'Niezawodne zasilanie części wspólnych.', img: '/assets/app_bloki_1783188247720.png' },
                  { title: 'Hale i Magazyny', desc: 'Zasilanie długich przemysłowych linii LED.', img: '/assets/app_hale_v2_1783188623293.png' },
                  { title: 'Obiekty sportowe', desc: 'Naświetlacze i taśmy na kortach i boiskach.', img: '/assets/app_sport_v2_1783188631467.png' },
                  { title: 'Infrastruktura i Mosty', desc: 'Oświetlenie odporne na drgania i wiatr.', img: '/assets/app_mosty_1783188351515.png' },
                  { title: 'Wiaty i Stolarka', desc: 'Zabezpieczenie termiczne w konstrukcjach drewnianych.', img: '/wiata_jezioro.png' }
                ].map((ap, idx) => (
                  <div key={idx} className="app-card-m" style={{ background: 'white', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 2px 5px rgba(0,0,0,0.03)' }}>
                    <img src={ap.img} alt={ap.title} style={{ width: '100%', height: '100px', objectFit: 'cover' }} />
                    <div className="app-card-info" style={{ padding: '8px' }}>
                      <h3 style={{ fontSize: '0.85rem', fontWeight: 700, margin: '0 0 2px 0', color: 'var(--c-heading)' }}>{ap.title}</h3>
                      <p style={{ fontSize: '0.7rem', color: '#666', margin: 0, lineHeight: 1.3 }}>{ap.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
