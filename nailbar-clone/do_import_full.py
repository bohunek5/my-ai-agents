...
# The import needs to run as a long-running background process
# Use AI1WM's built-in AJAX mechanism via secret key
code = r"""<?php
set_time_limit(0);
...
with open("do_import_full.php", "w") as f:
    f.write(code)
...
try:
    ...
    ftp.cwd(FTP_DIR)
    with open("do_import_full.php", 'rb') as f:
        ftp.storbinary('STOR do_import_full.php', f)
    ftp.quit()
    
    print("Script uploaded. Starting full import (this may take 10-20 minutes)...")
    req = urllib.request.Request("https://zeglarstwomazury.pl/do_import_full.php", headers={'User-Agent': 'Mozilla/5.0'})
    response = urllib.request.urlopen(req, timeout=600)
    print(response.read().decode('utf-8', errors='ignore'))
except urllib.error.HTTPError as e:
    print(f"HTTP Error {e.code}: {e.read().decode('utf-8', errors='ignore')[:500]}")
