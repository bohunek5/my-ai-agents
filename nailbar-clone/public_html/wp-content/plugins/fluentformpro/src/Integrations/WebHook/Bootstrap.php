...
class Bootstrap
{
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if($code < 300) {
            // it's a success
            do_action('ff_integration_action_result', $feed, 'success', 'Webhook payload has been fired. Status Code: '.$code);
            return;
        }
        do_action('ff_integration_action_result', $feed, 'failed', 'Webhook payload maybe failed to the target server. Status Code: '.$code);
    }
