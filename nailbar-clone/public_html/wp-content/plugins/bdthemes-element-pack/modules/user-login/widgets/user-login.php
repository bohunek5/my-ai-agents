...
class User_Login extends Widget_Base {
	...
	public function social_login() {
		...
		?>
		<?php if ( 'yes' == $settings['show_facebook_login'] or 'yes' == $settings['show_google_login'] ) : ?>
				...
				<span <?php echo $this->get_render_attribute_string( 'separator' ); ?>>
					<div class="bdt-social-wrapper bdt-flex bdt-flex-middle">
						<div class="bdt-social-login">
							...
							<?php if ( 'yes' == $settings['show_facebook_login'] ) : ?>
                                <div <?php echo $this->get_render_attribute_string( 'facebook-login' ); ?>>
								<a href="javascript:void(0);" data-appid="<?php echo esc_attr($gl_app_id) ?>" class="fb_btn_link"><span class="bdt-facebook-icon"><i class="ep-facebook fa-fw"></i></span> <?php echo esc_html('Facebook'); ?></a>
							</div>
							<?php endif; ?>

							<?php if ( 'yes' == $settings['show_google_login'] ) : ?>
                                <div class="bdt-google">
								<a href="javascript:void(0);" data-clientid="<?php echo esc_attr($gl_app_id) ?>" id="google_btn_link"><span class="bdt-google-icon"><i class="ep-google fa-fw"></i></span> <?php echo esc_html('Google'); ?></a>
							</div>
							<?php endif; ?>

							<?php if ( !$fb_secret or !$fb_app_id or !$gl_app_id ) : ?>
                                ...
                                <div class="bdt-alert-warning" bdt-alert>
                                    ...
                                    <p><?php echo sprintf( esc_html__( 'Ops! %1s %2s Missing. Please add them from: Element Pack Settings > API Settings > Social Login Access. ', 'bdthemes-element-pack' ), $fb_message, $google_app_message ); ?></p>
                                </div>
	...
	public function user_login_form() {
		...
		?>
        <form id="bdt-user-login<?php echo esc_attr($id); ?>" <?php echo $this->get_render_attribute_string( 'login-form-wrapper' ); ?> method="post">
            <input type="hidden" name="action" value="element_pack_ajax_login">
			<?php
			if($settings['show_recaptcha_checker']){
				echo do_action('element_pack_google_rechatcha_render', $this, 'button','onLoadElementorPackReCaptcha');
			}
			?>
            ...
            <input type="hidden" name="redirect_after_login" value="<?php echo esc_url($redirect_url) ?>" class="redirect_after_login"/>
