...
class Tabs extends Widget_Base {
	...
	protected function render() {
		...
		$id       = $this->get_id();
		$stickyClass = '';
		if( $settings['nav_sticky_mode'] == 'yes'){
                ...
                'tabs_sticky_data' => [
                    'data-settings' => [
                        wp_json_encode(array_filter([
                            ...
                            "navStickyOffset"   => $settings['nav_sticky_offset']['size'],
                        ])
                    ),
                    ],
                ],
        ...
        <div <?php echo $this->get_render_attribute_string( 'switcher-width' ); ?>>
            <div id="bdt-tab-content-<?php echo esc_attr( $id ); ?>" class="bdt-switcher bdt-switcher-item-content">
				<?php foreach ( $settings['tabs'] as $index => $item ) : ?>
                    <div content-id="<?php echo strtolower(preg_replace('#[ -]+#', '-', trim( preg_replace("![^a-z0-9]+!i", " ",esc_html($item['tab_title'])))))
; ?>">
                        <div>
							<?php
