		...
		<# if ( '' !== settings.image.url ) {
			...
			<div <# print(marker_wrapper); #>><#
				...
				<# _.each( settings.markers, function( item, index ) { 
					...
					if ( ( item.css_classes ).length != 0 ) {
						view.addRenderAttribute( 'marker', 'class', item.css_classes );
					}

					if ( ( item.css_id ).length != 0 ) {
						view.addRenderAttribute( 'marker', 'id', item.css_id, true );
					}
					...
					<a <# print( view.getRenderAttributeString( 'marker' ) ); #>>
						...
						<# } 
						...
						} else if ( item.select_type === 'image') {
							...
							var marker_image_url = elementor.imagesManager.getImageUrl( marker_image );
