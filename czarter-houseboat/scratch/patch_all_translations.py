...
# Fix missing "terraceAndOther" in PL
content = content.replace('roomBathroom: "Łazienka",', 'roomBathroom: "Łazienka", terraceAndOther: "Taras i Pozostałe",')
# Fix missing "bookOnlineBtn" in PL
content = content.replace('charterBtn: "730 067 027",', 'charterBtn: "730 067 027", bookOnlineBtn: "Rezerwuj Online",')


for lang in ['pl', 'en', 'de', 'es', 'lt']:
    # 1. Inject amenityNames
    # Find `lang: {`
    dict_str = "amenityNames: " + str(amenities_dict[lang]).replace("'", '"') + ","
    content = re.sub(rf'({lang}:\s*\{{\n\s*\.\.\.translationsLegal\.{lang},)', r'\1\n        ' + dict_str, content)

    # 2. Inject missing details fields
    if lang in details_addons:
        content = re.sub(rf"({lang}:.*?items:\s*\{{.*?)(\}})(\s*\}}\s*,)", r"\1" + details_addons[lang] + r"\2\3", content, flags=re.DOTALL)
    
    # 3. Inject missing hero fields
    if lang in hero_addons:
        content = re.sub(rf"({lang}:.*?hero:\s*\{{.*?)(\}})", r"\1, " + hero_addons[lang] + r"\2", content, flags=re.DOTALL)
